...
/* ==========================================
8. CERTIFICATE IMAGE VIEWER
========================================== */
function initCertificateViewer() {
  const certificateCards = document.querySelectorAll('.cert-card-trigger');
  ...
  const modalEmpty = document.getElementById('certificate-modal-empty');
  const closeButtons = document.querySelectorAll('[data-certificate-close]');

  if (!certificateCards.length || !modal || !modalTitle || !modalIssuer || !modalImage || !modalEmpty) return;
  ...
  function openModal(card) {
    const title = card.dataset.certificateTitle || 'Selected certificate';
    const issuer = card.dataset.certificateIssuer || '';
    const description = card.dataset.certificateDescription || '';
    const imageUrl = card.dataset.certificateImage || '';

    modalTitle.textContent = title;
    modalIssuer.textContent = description ? `${issuer} - ${description}` : issuer;

    if (imageUrl) {
      modalImage.src = imageUrl;
      modalImage.alt = `${title} certificate image`;
      ...
      modalEmpty.textContent = `No image uploaded yet for ${title}. Add it from the Django admin certification upload field.`;
    }
  ...
  certificateCards.forEach(card => {
    const headline = card.querySelector('.cert-name');
    if (headline) {
      headline.addEventListener('click', () => openModal(card));
    }
  });
...
/* ==========================================
9. DYNAMIC CONTACT FORM FEEDBACK (Django)
=========================================== */
function initContactForm() {
  ...
  form.addEventListener('submit', function(e) {
    ...
    const subjectVal = form.querySelector('#form-subject').value.trim();
    ...
    submitBtn.innerHTML =
      'Sending… <svg style="animation:spin 1s linear infinite;width:16px;height:16px;margin-left:8px;display:inline" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" opacity=".25"/><path fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/></svg>';
    ...
    .then(function(response) {
      return response.json().then(function(data) {
        if (!response.ok) {
          throw new Error(data.message || 'Server encountered an error.');
        }
        return data;
      });
    })
    .then(function(data) {
      ...
      showStatus('success', data.message || 'Thank you! Your message has been sent successfully.');
    })
    .catch(function(err) {
  ...
  function showStatus(type, msg) {
    ...
    setTimeout(function() { statusMsg.style.display = 'none'; }, 8000);
  }
}
